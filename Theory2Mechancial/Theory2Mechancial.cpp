...
void SaveMechanical(void)
{
	ofstream in;
	//写入
	in.open("../bridge/file/Mechanical.txt", ios::trunc); //ios::trunc表示在打开文件前将文件清空,由于是写入,文件不存在则创建
	for (int i = 0; i < s_StepNum[0]; i++)
	{
	...
	//创建标志位
	in.open("../bridge/file/MechanicalFlag.txt", ios::trunc); //ios::trunc表示在打开文件前将文件清空,由于是写入,文件不存在则创建
	in.close();
}
string GetTheory(void)
{
	fstream infile;
	infile.open("../bridge/file/Theory.txt");
	if (!infile) cout << "read Theory.txt error" << endl;

	string str;
	getline(infile, str);   //按行读取,遇到换行符结束
	infile.close();
	remove("../bridge/file/Theory.txt");
	return str;
}
bool IsTheorySended(void)
{
	char FileName[] = "../bridge/file/TheoryFlag.txt";
